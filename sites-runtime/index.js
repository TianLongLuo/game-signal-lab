...
const MAX_VOICE_ORGANIZE_BYTES = 16 * 1024;
const MAX_VOICE_ORGANIZE_TEXT = 4_000;
const MAX_AGENT_BYTES = 80 * 1024;
const MAX_AGENT_CONTENT_BYTES = 64 * 1024;
const MAX_KNOWLEDGE_BYTES = 256 * 1024;
const MAX_KNOWLEDGE_DOCUMENTS = 500;
const MAX_KNOWLEDGE_TITLE_LENGTH = 160;
...
async function route(request, env, ctx) {
  ...
  if (method === "GET" && path === "/api/me/knowledge") {
    const auth = await requireAuth(request, env);
    return json({ knowledge: await publicKnowledgeStatus(env, auth.id) });
  }
  if (method === "PUT" && path === "/api/me/knowledge") {
    return syncKnowledge(request, env, ctx);
  }
  if (method === "DELETE" && path === "/api/me/knowledge") {
    return clearKnowledge(request, env, ctx);
  }
  if (method === "POST" && path === "/api/agent/stream") {
    return streamAgent(request, env, ctx);
  }
  if (method === "POST" && path === "/api/voice/organize") {
    return organizeVoice(request, env, ctx);
  }
...
async function updateConsent(request, env) {
  ...
  await env.DB.batch([
    ...
    ...(body.accepted
      ? []
      : [
          env.DB.prepare("DELETE FROM user_rag_documents WHERE user_id = ?").bind(
            auth.id
          ),
        ]),
    auditStatement(
...
async function syncKnowledge(request, env, ctx) {
  const auth = await requireAuth(request, env);
  await requireCsrf(request, auth);
    ...
    statements.push(
      env.DB.prepare(
        `INSERT INTO user_rag_documents
          (id, user_id, external_id, kind, title, content, content_hash, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(
        `rag_${crypto.randomUUID()}`,
        auth.id,
        document.externalId,
        document.kind,
        document.title,
        document.content,
        await sha256(document.content),
        now,
        now
      )
    );
...
function validateKnowledgeDocuments(body) {
  ...
  if (!Array.isArray(body.documents) || body.documents.length > MAX_KNOWLEDGE_DOCUMENTS) {
    throw new HttpError(
      400,
      "invalid_knowledge_documents",
      `documents 必须是最多 ${MAX_KNOWLEDGE_DOCUMENTS} 条的数组。`
    );
  }
  const ids = new Set();
  return body.documents.map((document) => {
    ...
    if (!title || title.length > MAX_KNOWLEDGE_TITLE_LENGTH) {
      throw new HttpError(400, "invalid_knowledge_document", "个人档案标题不能为空或过长。");
    }
    if (!content || content.length > MAX_KNOWLEDGE_CONTENT_LENGTH) {
...
async function publicKnowledgeStatus(env, userId) {
...
async function retrieveUserKnowledge(env, userId, query) {
  ...
  values.push(MAX_KNOWLEDGE_RETRIEVED_DOCUMENTS);
  const rows = await env.DB.prepare(
    `SELECT kind, title, content
     FROM user_rag_documents
...
async function streamAgent(request, env, ctx) {
  ...
  try {
    upstream = await fetchDeepSeekWithRetry(endpoint, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          ...(privateContext
            ? [{ role: "system", content: privateContext }]
            : []),
          ...input.messages,
        ],
    ...
    const failure =
      error?.name === "AbortError"
        ? new HttpError(504, "provider_timeout", "DeepSeek 响应超时，请稍后重试。")
        : new HttpError(502, "provider_network_error", "暂时无法连接 DeepSeek，请稍后重试。");
    console.error("deepseek_request_failed", {
  ...
  if (!upstream.ok) {
    ...
    const failure = providerHttpError(upstream.status);
    console.error("deepseek_request_rejected", {
      providerStatus: upstream.status,
      providerCode: failure.code,
      model: config.model,
      providerRequestId: safeDiagnostic(
  ...
  if (!upstream.body || !upstreamContentType.startsWith("text/event-stream")) {
    clearTimeout(timeoutId);
    request.signal.removeEventListener("abort", abortFromClient);
    abortController.abort();
    ctx.waitUntil(
    ...
    console.error("deepseek_protocol_error", {
      providerStatus: upstream.status,
      providerCode: "provider_protocol_error",
      model: config.model,
      contentType: safeDiagnostic(upstreamContentType),
    });
    throw new HttpError(
      502,
...
async function organizeVoice(request, env, ctx) {
  const auth = await requireAuth(request, env);
  await requireCsrf(request, auth);
  if (!(await hasAgentAccess(env, auth))) {
    ctx.waitUntil(
  ...
  const body = await readJson(request, MAX_VOICE_ORGANIZE_BYTES);
  ...
  if (!text) throw new HttpError(400, "invalid_voice_text", "语音识别文字不能为空。");
  if (text.length > MAX_VOICE_ORGANIZE_TEXT) {
  ...
  if (!config?.enabled || !config.ciphertext || !config.iv || !DEEPSEEK_MODELS.has(config.model)) {
    throw new HttpError(503, "agent_not_configured", "Agent 服务尚未配置。");
  }
  ...
  const timeoutId = setTimeout(() => abortController.abort(), VOICE_ORGANIZE_TIMEOUT_MS);
  let upstream;
  try {
    upstream = await fetchDeepSeekWithRetry(endpoint, {
      method: "POST",
    ...
    clearTimeout(timeoutId);
    request.signal.removeEventListener("abort", abortFromClient);
    ctx.waitUntil(audit(env, request, auth.id, "voice.organize", "provider", "deepseek", "failure"));
    throw new HttpError(
      error?.name === "AbortError" ? 504 : 502,
      error?.name === "AbortError" ? "voice_organize_timeout" : "voice_organize_network_error",
      error?.name === "AbortError" ? "语音文字整理超时，请保留原识别文字后稍后重试。" : "暂时无法连接文字整理服务。"
    );
  }
  clearTimeout(timeoutId);
  request.signal.removeEventListener("abort", abortFromClient);
  if (!upstream.ok) {
    ctx.waitUntil(audit(env, request, auth.id, "voice.organize", "provider", "deepseek", "failure"));
    const failure = providerHttpError(upstream.status);
    console.error("voice_organize_request_rejected", {
      providerStatus: upstream.status,
      providerCode: failure.code,
      model: config.model,
    });
  ...
  ctx.waitUntil(audit(env, request, auth.id, "voice.organize", "provider", "deepseek", "success"));
  return withSecurity(json({ text: organized, provider: "deepseek" }));
}
...
async function synthesizeVoice(request, env, ctx) {
  ...
  const voice = typeof body.voice === "string" && MIMO_TTS_VOICES.has(body.voice)
    ...
    : "茉莉";
  const config = await env.DB.prepare(
    `SELECT enabled, model, ciphertext, iv
  ...
  const timeoutId = setTimeout(() => abortController.abort(), MIMO_TTS_TIMEOUT_MS);
  let upstream;
  try {
    ...
    clearTimeout(timeoutId);
    request.signal.removeEventListener("abort", abortFromClient);
    ctx.waitUntil(audit(env, request, auth.id, "voice.tts", "provider", "mimo_tts", "failure"));
    throw new HttpError(
      error?.name === "AbortError" ? 504 : 502,
  ...
  if (!upstream.ok) {
    ...
    const error = status === 401 || status === 403
      ? new HttpError(502, "tts_auth_failed", "语音服务 Key 无效或无权访问当前模型，请在后台重新配置。")
      : status === 429
        ? new HttpError(503, "tts_rate_limited", "语音服务请求过于频繁，请稍后重试。")
        : status >= 500
          ? new HttpError(503, "tts_provider_unavailable", "语音服务暂时繁忙，请稍后重试。")
          : new HttpError(502, "tts_provider_rejected", "语音服务未接受本次请求，请检查后台模型配置。");
    console.error("mimo_tts_request_rejected", {
  ...
  if (contentType.startsWith("audio/")) {
    ...
    return withSecurity(new Response(upstream.body, {
      status: 200,
      headers: { "Content-Type": contentType.split(";")[0] || "audio/mpeg", "Cache-Control": "no-store" },
    }));
  }
  let payload;
  try {
...
function providerHttpError(status) {
  ...
  if (status === 401 || status === 403) {
    return new HttpError(
      ...
      "DeepSeek API Key 无效或无权访问当前模型，请在后台重新配置。"
    );
  }
  if (status === 402) {
    return new HttpError(
  ...
  if (status >= 500) {
    return new HttpError(
      ...
      "DeepSeek 服务暂时繁忙，请稍后重试。"
    );
  }
  return new HttpError(502, "provider_error", "DeepSeek 暂时没有完成请求。");
}
...
async function routeAdminV1(request, env, ctx, path, url) {
  ...
  if (subpath === "/integrations/mimo-tts" && method === "GET") {
    return json(await publicMimoTtsConfig(env), 200, null, { admin: true });
  }
  if (subpath === "/integrations/mimo-tts" && method === "PATCH") {
    await requireCsrf(request, auth);
    return updateMimoTtsConfig(request, env, auth);
  }
...
async function adminOverview(env) {
  const [users, active, agent, audits, provider, tts] = await Promise.all([
    ...
    env.DB.prepare(
      "SELECT enabled, ciphertext FROM provider_configs WHERE provider = 'mimo_tts'"
    ).first(),
  ]);
  return json(
    {
      ...
      services: [
        ...
        {
          label: "MiMo V2.5 TTS",
          status: tts?.enabled && tts?.ciphertext ? "已配置" : "待配置",
          level: tts?.enabled && tts?.ciphertext ? "good" : "warn",
        },
      ],
    },
    200,
...
async function updateMimoTtsConfig(request, env, auth) {
  const body = await readJson(request);
  if (typeof body.enabled !== "boolean") {
    throw new HttpError(400, "INVALID_ENABLED", "enabled 必须是布尔值。");
  }
  ...
  const existing = await env.DB.prepare(
    "SELECT ciphertext, iv, created_at FROM provider_configs WHERE provider = 'mimo_tts'"
  ).first();
  let encrypted = existing?.ciphertext
    ? { ciphertext: existing.ciphertext, iv: existing.iv }
    : null;
...
function normalizeVoiceOrganizedText(value) {
  ...
  return value
    ...
    .replace(/\s*```\s*$/i, "")
    .trim()
    .slice(0, MAX_VOICE_ORGANIZE_TEXT);
}
