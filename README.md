...
| 可选平台平面 | 账号、会员、Agent 授权、外部 AI 明示同意、管理员配置与最小审计 | 由同源 Node/SQLite 或 Sites Worker/D1 服务处理；只有用户在 Agent 界面明确提交的文本会发送给 DeepSeek |
...
平台审计只记录账号/管理/Agent 操作的最小元数据：动作、资源、结果、固定原因代码、请求 ID 和时间。它不记录关系日记、Agent prompt/reply、系统提示词、密码、API Key、IP 地址或 User-Agent。管理员因此看不到本地关系内容，也看不到 Agent 对话正文。
