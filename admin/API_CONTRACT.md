...
管理员可以看到用户账户、授权、最小操作审计，以及用户已明确同意保存的加密 Agent/故事消息。管理接口不返回 RAG 文档、档案正文或检索片段；个人知识库接口只对当前登录用户开放，服务端始终以会话 `user_id` 做所有权过滤。
...
查询参数：`page`、`pageSize`，以及可选的 `userId`、`channel=all|agent|story`。返回按时间倒序的用户消息和模型回复。正文从 AES-256-GCM 加密记录中按需解密；每次调用会写入 `admin.messages.read` 审计事件。该接口不返回系统提示词、RAG 档案正文、API Key 或其他用户认证信息。
