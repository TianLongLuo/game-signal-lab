...
- 每个新注册普通账户自动获得 50 次 DeepSeek Agent/语音整理调用额度，计数保存在 `agent_usage_quotas`。额度用完后，管理员可在“用户与授权”里启用该账户的 Agent 权限，切换为持续可用；后台用户卡片显示已用、总量和剩余次数。
- Agent 同时受当前外部 AI 同意、provider 开关与全局开关、账户状态、50 次包含额度或个人持续授权、每用户频率与并发、全局并发约束。一次额度只在请求完成校验、即将调用 DeepSeek 时原子扣减；ASR/TTS 不计入该 DeepSeek 调用额度。
